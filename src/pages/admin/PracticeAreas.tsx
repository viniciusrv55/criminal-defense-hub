...
const PracticeAreasAdmin = () => {
  ...
  const startEdit = (area: PracticeArea) => {
    setForm({
      ...
      icon_name: area.icon_name ?? 'Gavel',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingId) {
      ({ error } = await supabase.from('practice_areas').update(data).eq('id', editingId));
    } else {
      ({ error } = await supabase.from('practice_areas').insert({ ...data, sort_order: areas.length }));
    }
        ...
        <form onSubmit={handleSubmit} className="mb-8 p-6 bg-card rounded-xl border border-border space-y-4">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Textarea
                value={form.icon_svg}
                onChange={(e) => setForm((p) => ({ ...p, icon_svg: e.target.value }))}
                placeholder="Cole o SVG aqui..."
                rows={2}
          ...
          {areas.map((area) => (
            ...
            >
              <div className="flex items-center gap-4">
                ...
                >
                  {area.icon_svg ? (
                    <div
                      className="w-5 h-5"
                      style={{ color: area.icon_color }}
                      dangerouslySetInnerHTML={{ __html: area.icon_svg }}
                    />
                  ) : (
                    <span style={{ color: area.icon_color }} className="text-xs font-bold">
                      {area.icon_name?.charAt(0) ?? '?'}
                    </span>
                  )}
                </div>
                <div>
              ...
              <div className="flex items-center gap-2">
                <span className={`text-xs px-2 py-0.5 rounded-full ${area.active ? 'bg-green-500/10 text-green-500' : 'bg-muted text-muted-foreground'}`}>
                  {area.active ? 'Ativa' : 'Inativa'}
                </span>
                <Button variant="ghost" size="sm" onClick={() => startEdit(area)}>
