        ...
        <div className="flex items-center justify-between h-20">
          ...
          <div className="hidden lg:block">
            <Button variant="gold" size="lg" asChild>
              <Link to="/contato"><Phone className="w-4 h-4" />Fale Conosco</Link>
            </Button>
          </div>
        ...
        {isMenuOpen && (
          <div className="lg:hidden py-4 border-t border-border animate-fade-up">
            <div className="flex flex-col gap-4">
              {navLinks.map(link => (
                <Link key={link.href} to={link.href} onClick={() => handleNavClick(link.href)} className="text-base font-medium text-muted-foreground hover:text-foreground transition-colors py-2">
                  {link.label}
                </Link>
              ))}
